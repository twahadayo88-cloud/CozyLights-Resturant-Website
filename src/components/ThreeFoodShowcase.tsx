...
export default function ThreeFoodShowcase() {
  ...
  const dishes = [
    {
      ...
      ingredients: ["Miyazaki A5 Wagyu", "Saffron Dashi", "24K Gold Leaf", "Organic Shoyu Egg", "Plucked Sakura Petals"],
      color: 0xffd700, // Gold
    ...
    {
      name: "Sakura Royal Shimmering Elixir",
      nativeName: "桜ロワイヤル・カクテル",
      ingredients: ["Sakura Botanical Essence", "Elderflower Liqueur", "Champagne Bubbles", "Liquid 24K Gold Sparkles"],
      color: 0xff69b4, // Cherry Blossom Pink
  ...
  useEffect(() => {
    ...
    const pointLightAmbient = new THREE.PointLight(0xff69b4, 2, 15);
    ...
    // B. Floating Ingredient Particles (Wagyu blocks, sakura petals, and pearls)
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const customMat = new THREE.MeshStandardMaterial({
        color: i % 2 === 0 ? 0xffd700 : dishes[activeDish].color,
        roughness: 0.3,
        metalness: 0.9
      });
  ...
  return (
    <section id="specials-cube" className="relative py-24 bg-[#0A0A0A] border-t border-b border-[#F9F5EE]/10 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          <span className="text-yellow-500 font-serif tracking-[0.3em] text-[10px] uppercase block mb-3 italic">Interactive 3D Art</span>
          <h2 className="text-4xl md:text-5xl font-serif text-white uppercase leading-[1.1]">
            Rotating Luxury <span className="text-yellow-500 font-serif italic normal-case tracking-normal">3D Showcases</span>
          </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            {dishes.map((dish, idx) => (
              <button
                ...
                className={`w-full text-left p-6 rounded-none transition-all duration-500 border relative group cursor-pointer ${
                  activeDish === idx
                    ? "bg-[#081120]/80 border-yellow-500/50 shadow-[0_0_20px_rgba(255,215,0,0.05)]"
                    : "bg-neutral-900/10 border-[#F9F5EE]/10 hover:border-neutral-700"
                }`}
              >
                {/* Active glow pointer */}
                {activeDish === idx && (
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <span className="text-[8px] font-mono uppercase border border-yellow-500/30 text-yellow-500 px-2.5 py-0.5 rounded-none tracking-widest">
                      0{idx + 1} / FEATURED
                    </span>
                    <h3 className="text-lg font-serif text-white mt-3 group-hover:text-yellow-500 transition-colors duration-300 uppercase tracking-tight">
                      {dish.name}
                    </h3>
                    <p className="text-[10px] text-neutral-500 font-mono tracking-wider mt-0.5">{dish.nativeName}</p>
                  </div>
                  {activeDish === idx && (
                    <Sparkles className="w-4 h-4 text-yellow-500 animate-pulse mt-1" />
                  )}
                </div>
                ...
                {activeDish === idx && (
                  <div className="mt-4 pt-4 border-t border-[#F9F5EE]/10 flex justify-between items-center text-[10px] text-yellow-500 font-mono tracking-wide">
                    <span>{dish.spec}</span>
          ...
          <div className="lg:col-span-7 relative bg-neutral-950/40 border border-[#F9F5EE]/10 rounded-none p-6 min-h-[480px] flex flex-col justify-between overflow-hidden shadow-2xl">
            ...
            <div className="flex justify-between items-center text-[10px] font-mono text-neutral-500 tracking-widest">
              ...
              <span className="uppercase text-yellow-500">Miyazaki Alignment [OK]</span>
            </div>
            ...
            <div ref={containerRef} className="w-full h-[350px] relative cursor-grab active:cursor-grabbing flex justify-center items-center">
              <span className="text-neutral-600 font-mono text-xs uppercase tracking-widest">Aligning WebGL Matrix...</span>
            </div>
            ...
            <div className="border-t border-[#F9F5EE]/10 pt-4 mt-auto">
              <div className="flex flex-wrap gap-2 items-center">
                ...
                {dishes[activeDish].ingredients.map((ing, i) => (
                  <span
                    key={i}
                    className="text-[9px] font-mono text-neutral-300 bg-neutral-900/30 border border-[#F9F5EE]/10 px-3 py-1 rounded-none uppercase tracking-wider group hover:border-yellow-500/20"
                  >
                    ✦ {ing}
                  </span>
                ))}
